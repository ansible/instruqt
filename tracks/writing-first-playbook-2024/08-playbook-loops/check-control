...
target_file="${target_directory}/loop_users.yml"
solve_file="/tmp/first-101/solve_17_system_setup_loops.yml"
...
if [ -f "$target_file" ]; then
    ...
    if [ "$file_content" != "$solve_content" ]; then
        fail-message "Challenge 1.7 - The 'loop_users.yml' file does not have the proper content. Please check the file and try again."
    fi
else
    fail-message "Challenge 1.7 - The 'loop_users.yml' file does not exist in the specified directory."
fi
