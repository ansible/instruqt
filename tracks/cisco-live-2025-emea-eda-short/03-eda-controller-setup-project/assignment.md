...
id: rpxvyk0kyzuk
type: challenge
title: 3 - Automation Decisions - setting up Telemetry
...
---
3 - 📊  Automation Decisions - setting up Telemetry
===
...
In the previous exercises you expermiented with `ansible-rulebook` and `kafka`. In this exercise we will be adding `telegraf` and interact with our network devices.
...
![Feb-07-2025_at_00.05.18-image.png](../assets/Feb-07-2025_at_00.05.18-image.png)

3. No problem! We will configure the switch to enable telemetry and to stream data to our telemetry collector.

4. Switch to the [button label="VS Code"](tab-5) tab.

5. In the **Explorer** sidebar, right click on the `playbooks` directory and click on `New File`. Name the file `configure_telemetry.yml` and press **Enter**.

6. Populate the `configure_telemetry.yml` file with the following content and save it.
...
7. Switch to the [button label="Terminal"](tab-4) tab
...
8. Switch to the  [button label="cisco"](tab-3)  tab,
9. Connect to the device if you are not in: `ssh  admin@cisco`
10.  and execute the following command to verify that telemetry configurations have been pushed.

  ```bash
  show running-config | section telemetry
  ```

9. The output of the above command should look like this:

![Feb-07-2025_at_00.13.13-image.png](../assets/Feb-07-2025_at_00.13.13-image.png)
...
![Feb-07-2025_at_00.14.09-image.png](../assets/Feb-07-2025_at_00.14.09-image.png)
